# res://scripts/masks/mask_visual.gd
class_name MaskVisual
extends Node2D

@export var radius: float = 200.0
@export var color: Color = Color(1.0, 0.2, 0.2, 0.3)  # Цвет заливки
@export var outline_color: Color = Color(1.0, 0.4, 0.4, 0.6)  # Цвет контура
@export var outline_width: float = 3.0
@export var pulse_speed: float = 2.0
@export var pulse_min_scale: float = 0.95
@export var pulse_max_scale: float = 1.05
@export var pulse_alpha: bool = true  # Пульсирует ли прозрачность
...
func _draw():
	...
	# Изменяем альфа для пульсации (опционально)
